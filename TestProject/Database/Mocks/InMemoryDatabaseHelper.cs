...
namespace UnitTests.Database.Mocks
{
    ...
    public class InMemoryDatabaseHelper
    {
        ...
        public DataTable GetBooksByFilter(int? authorId, int? genreId, decimal? minRating, decimal? maxRating)
        {
            ...
            {
                // avoid using DataRow.Field<T> extension to keep tests simple
                decimal rating = r["Rating"] == DBNull.Value ? 0m : (decimal)r["Rating"];
                string genres = r["Genres"] as string ?? string.Empty;
        ...
        public (bool Success, string Message, int UserID) RegisterUser(string username, string email, string password, string fullName)
        {
            ...
            {
                string existingUsername = r["Username"] as string ?? string.Empty;
                string existingEmail = r["Email"] as string ?? string.Empty;
        ...
        public bool UserExists(string username)
        {
            ...
            {
                string existingUsername = r["Username"] as string ?? string.Empty;
                if (string.Equals(existingUsername, username, StringComparison.OrdinalIgnoreCase)) return true;
            }
            return false;
        }
        ...
        public bool EmailExists(string email)
        {
            ...
            {
                string existingEmail = r["Email"] as string ?? string.Empty;
                if (string.Equals(existingEmail, email, StringComparison.OrdinalIgnoreCase)) return true;
            }
            return false;
        }
    }
